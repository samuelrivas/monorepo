...
# FIXME: this interface is unnecessary, but is here for compatibility with the
# old fashioned way of building this package set, while we migrate it to
# flakes. We import this from two places, the flake.nix file and the
# nix/default.nix file. The old usage was via nix/default.nix, still documented
# in the README, we will be migrating to flakes and removing nix/default.nix
# eventually.
{ nixpkgs-upstream, nixpkgs-patched, nixpkgs-stable }:
  let
  ...
  pkgs-sam = {
    ...
    pkgs-upstream = nixpkgs-upstream;

    # My own fork of nixpkgs, for patches that aren't merged
    pkgs-patched = nixpkgs-patched;
