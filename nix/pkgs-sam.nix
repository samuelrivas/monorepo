  ...
  perSystem = { config, self', pkgs, ... }:
    let
      ...
      selfish = {
        ...
        # callPackageWith instead of polluting the top level namespace with
        # them. We do add all our pakcages to the top level namespace though
        # inherit libs-sam;

        haskell-lib = import ./lib/with-system/haskell.nix {
          pkgs = pkgs // self'.packages;
        };

        haskellPackages = pkgs.haskellPackages.override {
          overrides = h-pkgs: h-prev:
            let
              h-package = h-pkgs.callPackage;
            in {
              inherit (selfish.haskell-lib) haskell-pkg haskell-lib-pkg;
        ...
        derivations-sam = {
          ...
          adventlib = selfish.haskellPackages.adventlib;
          adventlib-old-1 = selfish.haskellPackages.adventlib-old-1;
          boardgamer = selfish.haskellPackages.boardgamer;
          boollib = selfish.haskellPackages.boollib;
          clean-clocks = selfish.haskellPackages.clean-clocks;
          hashcode-photoalbum = selfish.haskellPackages.hashcode-photoalbum;
          low-battery = selfish.haskellPackages.low-battery;
          mk-conf-file = selfish.haskellPackages.mk-conf-file;
          monad-emit = selfish.haskellPackages.monad-emit;
          name-generator = selfish.haskellPackages.name-generator;
          onirim-helper = selfish.haskellPackages.onirim-helper;
          parselib = selfish.haskellPackages.parselib;
          perlude = selfish.haskellPackages.perlude;
          searchlib = selfish.haskellPackages.searchlib;
          ...
          udp-cat = pkgs.callPackage ./pkgs/applications/networking/tools/udp-cat { };

          # Contests, puzzles, etc
          # ======================
          adventofcode-2019 = selfish.haskellPackages.adventofcode-2019;
          adventofcode-2020 = selfish.haskellPackages.adventofcode-2020;
          adventofcode-2021 = selfish.haskellPackages.adventofcode-2021;
        };
        all-pkgs-sam = pkgs.linkFarm "pkgs-sam" (
          pkgs.lib.mapAttrsToList
            (n: v: { name = n; path = v; })
            selfish.derivations-sam
        );
      };
    in {
      config.packages =
        selfish.derivations-sam
        // {
          inherit (selfish) all-pkgs-sam;
          default = selfish.all-pkgs-sam;
        };
      config.devShells =
        builtins.mapAttrs
          (name: value:
