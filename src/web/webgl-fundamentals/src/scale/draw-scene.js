...
function setScaleUniform(gl, program, x, y) {
    var location = gl.getUniformLocation(program, "u_scale");
    gl.uniform2f(location, x, y);
}
...
function redrawGl(gl, graphicsState) {
    ...
    setScaleUniform(gl, program, scale[0], scale[1]);
...
function getUiSliders() {
    ...
    var sliderScaleX = getSlider("x-scale");
    var sliderScaleY = getSlider("y-scale");
...
function setUI(gl, graphicsState) {
    ...
    sliders.xScale.oninput = () => updateSlider(gl, graphicsState, sliders);
    sliders.yScale.oninput = () => updateSlider(gl, graphicsState, sliders);
...
function getScale(sliders) {
    return [sliders.xScale.valueAsNumber, sliders.yScale.valueAsNumber];
}
...
function updateSlider(gl, graphicsState, sliders) {
    ...
    graphicsState.scale = getScale(sliders);
    redrawGl(gl, graphicsState);
}
